 ...
 *   요청: {"agentId":"agent01","reqId":"req01","time":"20250303100000","dataType":"P","dataValue":0,"latency":120}
 *   응답: {"result":"OK"}
 *
 * POST /perf  - 모델 성능 지표 조회
 *   요청: {"modelName":"qwen-2.5","time":"2025030310"}
 *   응답: {"correct":3,"total":5,"latency":115.5}
 */
public class MyServlet extends HttpServlet {
    ...
    private void handleMonitor(JsonObject body, JsonObject result) {
        ...
        double latency   = body.get("latency").getAsDouble();

        DataStore.INSTANCE.add(
            new DataStore.AgentRecord(agentId, reqId, time, dataType, dataValue, latency)
        );
    ...
    private void handlePerf(JsonObject body, JsonObject result) {
        ...
        double avgLatency = latencyCount > 0 ? latencySum / latencyCount : 0;

        result.addProperty("correct", correct);
        result.addProperty("total", total);
        result.addProperty("latency", avgLatency);
    }
}
